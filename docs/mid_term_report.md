...
### 5.1 v1 → v2 → v3 → v4 실험 이력

| 지표 | v1 (16건) | v2 (16건) | v3 (50건) | **v4 (50건)** |
|------|---------|---------|---------|--------------|
| 정확도 | 75.0% | 93.8% | 84.0% | **94.0%** |
| F1 Macro | 0.844 | 0.933 | 0.788 | **0.940** |
| FPR | 0.0% | 0.0% | 16.7% | **0.0%** |
| FNR | 0.0% | 0.0% | 0.0% | **0.0%** |
| 평균 지연 | 3,984ms | 1,907ms | 1,837ms | **1,773ms** |

- **v1→v2**: 우선순위 규칙 추가 → +18.8%p (다중 카테고리 오류 해소)
- **v2→v3**: 50케이스로 확장 → 약점 발견 (S5 F1 0.333, FPR 16.7%)
- **v3→v4**: 시스템 프롬프트 v3 개선 → **S5 F1 1.000, FPR 0.0%** 달성 (+10%p)

### 5.2 v4 카테고리별 성능 (50케이스, 시스템 프롬프트 v3 적용)

| 카테고리 | F1 (v3) | **F1 (v4)** | 변화 | 잔여 오류 |
|---------|--------|-----------|------|---------|
| safe | 0.909 | **1.000** | +0.091 | 없음 |
| S1 (PII) | 0.833 | **0.842** | +0.009 | EDGE-003 |
| S2 (Credential) | 1.000 | **1.000** | — | 없음 |
| S3 (Prompt Injection) | 0.857 | **1.000** | +0.143 | 없음 |
| S4 (내부 유출) | 0.824 | **0.800** | -0.024 | S4-001, S4-007 (PII 포함으로 S1 판정) |
| **S5 (시스템 추출)** | **0.333** | **1.000** | **+0.667 ★** | 없음 |
